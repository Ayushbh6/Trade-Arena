...
export default function Home() {
  const { events, isConnected, isServerReady, isRunning, connectAndRun, disconnect, tokenCounts, startCycle, stopCycle } = useAgent();
  ...
  return (
    <main className="h-screen w-screen bg-black text-white overflow-hidden flex flex-col font-sans selection:bg-indigo-500/30">
      {/* Header - Sleek Glass */}
      <header className="h-14 flex-none border-b border-white/10 flex items-center justify-between px-6 bg-black/50 backdrop-blur-xl sticky top-0 z-50 relative">
        <div className="flex items-center gap-3">
           <div className="h-8 w-8 bg-white/10 rounded-xl flex items-center justify-center border border-white/5 shadow-inner">
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
            <RunControl isRunning={isRunning} onStart={startCycle} onStop={stopCycle} />
        </div>
